...
$(document).ready(function() {
  ...
  $("#msg").keypress(function(e){
    if (e.which !== 13) { //Checking if NOT "Enter" key
      if (typing === false && myRoomID !== null && $("#msg").is(":focus")) { //":focus" is jquery's pseudo selector. pure js alternative: this === document.activeElement.
        typing = true;
        socket.emit("typing", true); //Send "typing" to server.js
      } else {
        clearTimeout(timeout); //Always have to reset the timer after the typing stops. Logical.
        timeout = setTimeout(timeoutFunction, 5000); //In 5 seconds, timeoutFunction will set typing to false and emit the status.
      }
    }
  });

  //If "typing" is emitted, server sends "isTyping" back
